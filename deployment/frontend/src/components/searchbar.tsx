...
    onPredictionMade: (price: number | null) => void;
}
...
  // ⭐ FIX 1: Removed the internal prediction state: 
...
      onPredictionMade(null); // Signal loading/clear previous result
...
          // ⭐ FIX 2: Call onPredictionMade with 0 on API error
          onPredictionMade(0);
...
        
        // Check if the prediction data is a valid number before reporting
        if (typeof data.prediction === 'number' && !isNaN(data.prediction)) {
          // ⭐ FIX 3: Call onPredictionMade with the result on success
          onPredictionMade(data.prediction);
          // Removed: setPrediction(data.prediction)
        } else {
          console.error("Prediction result was invalid or NaN:", data);
          console.log(data)
          // ⭐ FIX 4: Call onPredictionMade with 0 on invalid result
          onPredictionMade(0);
          // Removed: setPrediction(0)
        }
  
      } catch (error) {
        console.error("Prediction error:", error)
        // ⭐ FIX 5: Call onPredictionMade with 0 on network/exception error
        onPredictionMade(0);
      }
    }
...
    // ... (FieldSegment logic remains unchanged) ...
...
        ref: inputRef,
        // Remove readOnly from commonProps to prevent disruptive re-renders
...
          )
        default:
             // ⭐ FIX: Add readOnly back here for the COUNTY field only, based on its value.
